...
class Tree(object):
    ...
    def addRoutine(self,routine,Rroot,count):
        """功能：根据实物routine 递归构造树，Rroot是树的根节点，count是routine出现的次数(构造条件FP_Tree的时候有用)"""
        if len(routine) <= 0:           #如果书屋为空，则终止
            return
        elem = routine.pop(0)
        if elem in Rroot.children:      #如果实物中的元素在树的已有路径上
            nextNode = Rroot.children[elem] #如果实物中的元素在树的已有路径上，则不用新建路径
        else:
        ...
        if nextNode not in self.itemTable[elem]: #如果下一个节点是新建的，则把他压入头节点表中
